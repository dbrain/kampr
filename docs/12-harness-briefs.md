...
The harness omp forked ([#490](./03-probe-log.md)): the same record grammar and session path, and none of omp's handles. What it has instead was measured on a throwaway herdr session running it against a local endpoint, and in the operator's own live sessions. Rows [#542](./03-probe-log.md)–[#546](./03-probe-log.md).

| Fact | How | Confidence |
|---|---|---|
| It writes nothing to disk that ties a process to a session, but **every command it runs carries `PI_SESSION_FILE` in its environment**, and the kernel keeps that | pi's own `resolveSpawnContext` (`bash.js`), and the live `/proc/<pid>/environ` of the pane's pi and of the commands it ran ([#542](./03-probe-log.md)) | **measured** |
| The pane's own pi process carries **no** `PI_*` variable at all — the session id is runtime state, not environment | the pane's process read in a live session | **measured** |
| The subagents extension puts the root session's id into the root process's in-memory environment; the kernel's view of that process never shows it, but every child spawned after the set carries it at exec time, and a subagent's runner inherits it and must not overwrite it | the extension's source, and the tool children's environments ([#543](./03-probe-log.md)) | **measured** |
| Its session bucket is `--<absolute-path>--`, not omp's `-<home-relative>` — so the path is taken verbatim from the environment and only *proved* to sit under `sessions/`, never re-derived | `~/.pi/agent/sessions` listed against the environment's answer | **measured** |
| While a turn is in flight the composer's top border is `── ⠹ Working ──…` — ten braille frames on an 80 ms cadence — and idle it is a plain run of dashes; a retry or a compaction spins on the status line above the border instead | `pane read` live in both states, and pi's own `renderTopBorder` / `showStatusIndicator` ([#544](./03-probe-log.md)) | **measured** |
| herdr's `pi` manifest is `contains ["Working..."]`, which matches nothing pi draws — `agent_status` says `idle` **during** a working turn and after it | `pane.list` sampled mid-turn and after, on the live session | **measured** |
| Queued prompts are on the screen — `Steering: …` and `Follow-up: …`, one column in, cut to the row rather than wrapped, a `↳ … to edit all queued messages` hint under the last — and nowhere in the file until delivered | pi's `updatePendingMessagesDisplay`, and a live capture ([#545](./03-probe-log.md)) | **measured** |
| `/proc/<pid>/environ` reads as an **empty success** while the process is still mid-exec — 198/200 and 196/200 zero-byte reads immediately after spawn, 0/50 five milliseconds on; `stat` and `comm` have no such window | a spawn-and-read harness, two runs ([#546](./03-probe-log.md)) | **measured** |
| A custom spinner an extension installs reads `None`, not `idle` — the read keys on the ten frames and the dash run, not on the word `Working` | the frame set is `pi-tui`'s `DEFAULT_FRAMES`, and the read is vocabulary, not layout | **by construction** |

Because the environment names the session only while a tool is in flight, a pi pane between tools names nothing at all — and a resolution that fell back to the working directory there would latch onto whoever ran last in it, the exact “most recent session from this dir” the operator reported. So a pi pane keeps the session it was last seen on, for as long as the pane is the same (same agent, same directory, harness still running), until the environment names a different one. The rule is applied in two places that must not disagree: the conversation pump, which serves the transcript, and the herd model, which sets the panel's title and `has_conversation` — a title that named a different session than the one being served would be the latch again, one rebuild wide.
...
| **pi** | Done | The pane is the session its processes name in their environment ([#542](./03-probe-log.md), [#543](./03-probe-log.md)): the node walks the pipeline's `/proc/<pid>/environ` before any disk walk, and a hit skips the held-descriptor and tty-crumb walks pi never pays. The run state herdr has no working manifest for ([#490](./03-probe-log.md)) is read off the screen — the spinner in the composer's top border, the ten frames `pi_status` accepts in the border or the status line above it — and published as the additive `Facets.status`, which the conversation's working strip lights from where the herd's status cannot ([#544](./03-probe-log.md)). The queue is read off the screen the same way ([#545](./03-probe-log.md)). A spawn *quoted* in a result other than `task` mints no card, and the task's own acknowledgement still mints; both halves are tested. `environ`'s mid-exec empty read is measured and polled out of the tests rather than trusted ([#546](./03-probe-log.md)). The environment handle is gone the moment the naming process is ([#542](./03-probe-log.md)), so a pi pane is the session it was last seen on between tools — the node keeps the announced session for as long as the pane is the same, until the marker names a different one — which is what stops the conversation from re-resolving by directory and latching onto whoever ran last in it; a harness whose marker answers on every tick has nothing to remember and is not sticky |
