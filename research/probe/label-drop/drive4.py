...
def main():
    ...
    start()
    try:
        print("=== A: how late must the title be written to be adopted? ===", flush=True)
        for delay in (0.15, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.15, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50):
            fifo = os.path.join(SP, "f%.1f.fifo" % delay)
            if os.path.exists(fifo): os.remove(fifo)
            os.mkfifo(fifo)
            pane = call("workspace.create", {"label": "d%.1f" % delay, "cwd": SP})["root_pane"]["pane_id"]
            time.sleep(1.5)
            call("pane.send_text", {"pane_id": pane, "text": "%s/claude %s/fake_agent3.py %s %s/x.log %s/sleep '%s' %s\r"
                                    % (BIN, SP, fifo, SP, BIN, WORKING, delay)})
            time.sleep(delay + 8.0)
            s = snap(pane)
            print("  title at +%.1fs -> agent=%r status=%r oscb=%r fb=%r rule=%r"
                  % (delay, s.get("agent"), s.get("status"), s.get("oscb"), s.get("fb"), s.get("rule")), flush=True)
            with open(fifo, "w") as f: f.write("quit\n")
            time.sleep(0.6)
            call("pane.close", {"pane_id": pane}, quiet=True)
            os.remove(fifo)

        return
        fifo = os.path.join(SP, "fb.fifo")
        if os.path.exists(fifo): os.remove(fifo)
        os.mkfifo(fifo)
        pane = call("workspace.create", {"label": "db", "cwd": SP})["root_pane"]["pane_id"]
        time.sleep(1.5)
        call("pane.send_text", {"pane_id": pane, "text": "%s/claude %s/fake_agent3.py %s %s/x.log %s/sleep '%s' 5\r"
                                % (BIN, SP, fifo, SP, BIN, WORKING)})
        time.sleep(11)
        print("  before release: %s" % json.dumps(snap(pane), ensure_ascii=False), flush=True)
        print("  release_agent ->", json.dumps(call("pane.release_agent", {"pane_id": pane}, quiet=True))[:200], flush=True)
        for w in (0.5, 1.0, 2.0, 4.0, 8.0):
            time.sleep(w)
            print("    +%.1f %s" % (w, json.dumps(snap(pane), ensure_ascii=False)), flush=True)
        print("  clear_agent_authority ->", json.dumps(call("pane.clear_agent_authority", {"pane_id": pane}, quiet=True))[:200], flush=True)
