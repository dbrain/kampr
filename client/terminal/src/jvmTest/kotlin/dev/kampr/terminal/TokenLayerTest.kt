...
// The same rule shared enforces on itself: phosphor / warm / brutalist stay one attribute away,
// and a literal anywhere outside the token layer breaks that quietly enough that only a
// screenshot in a second theme would catch it.
class TokenLayerTest {
    ...
    private fun sourceRoot(): File {
        ...
        repeat(4) {
            val candidate = File(dir, "src/commonMain/kotlin/dev/kampr/terminal")
            if (candidate.isDirectory) return candidate
            dir = dir.parentFile ?: return@repeat
        }
        fail("could not locate terminal common sources from ${File(".").absolutePath}")
    }

    @Test
    fun noColourFontOrRadiusLiteralInTheRenderer() {
