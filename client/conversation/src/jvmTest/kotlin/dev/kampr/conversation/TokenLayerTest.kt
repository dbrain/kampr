...
// The same guard :shared keeps over its own sources. Syntax highlighting is where a hardcoded
// palette normally creeps into a renderer, so it is the one that most needs the rule.
class TokenLayerTest {
    private val offenders = listOf(
        ...
        Regex("""FontFamily\.(Monospace|SansSerif|Serif|Cursive|Default)""") to "platform font family",
    ...
    private fun sourceRoot(): File {
        ...
        repeat(4) {
            val candidate = File(dir, "src/commonMain/kotlin/dev/kampr/conversation")
            if (candidate.isDirectory) return candidate
            dir = dir.parentFile ?: return@repeat
        }
        fail("could not locate conversation common sources from ${File(".").absolutePath}")
    }
