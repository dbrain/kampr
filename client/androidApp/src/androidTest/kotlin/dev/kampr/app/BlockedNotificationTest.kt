...
class BlockedNotificationTest {
    ...
    // stands until another replaces it.
    @Test
    fun clearingTakesThePromptDown() {
        postBlockedNotification(context, "claude is blocked", "wants your approval", "pane-7")
        shade()

        clearBlockedNotification(context)

        repeat(50) {
            if (ours().isEmpty()) return
            Thread.sleep(100)
        }
        throw AssertionError("the prompt was still in the shade 5s after it was answered")
    }

    // Answering one of two leaves the other one waiting, and its notification has to say so.
    @Test
    fun aResyncRewritesAPromptThatIsStillShowing() {
        postBlockedNotification(context, "2 agents need you", "one · two", null)
        shade("2 agents need you")

        postBlockedResync(context, "codex needs you", "Apply the patch?", "pane-2")

        val posted = shade("codex needs you").notification
        assertEquals("Apply the patch?", posted.extras.getString("android.text"))
        assertEquals("a resync replaces the prompt rather than stacking beside it", 1, ours().size)
    }

    // Somebody who swiped the prompt away has already dealt with it. A quieter copy of what they
    // dismissed is the app arguing with them — and it is what a naive "just post the new summary"
    // would do on every answered pane.
    @Test
    fun aResyncDoesNotConjureAPromptThatWasDismissed() {
        manager.cancel(BLOCKED_NOTIFICATION_ID)

        postBlockedResync(context, "codex needs you", "Apply the patch?", "pane-2")

        Thread.sleep(500)
        assertEquals("nothing was on the screen, so there was nothing to correct", 0, ours().size)
    }
