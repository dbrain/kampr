...
class KamprPushService : PushService() {
    ...
    override fun onMessage(message: PushMessage, instance: String) {
        ...
        // A payload with no `kind` is from a node older than the second slot, and such a node only
        // ever sent the blocked one. Reading it as anything else would put a question in the wrong
        // shade entry.
        val kind = NoteKind.of(note?.text("kind"))
        val title = note?.text("title") ?: "An agent needs you"
        val body = note?.text("body") ?: "Open Kampr to see which"
        val pane = note?.text("pane")
        // Nothing outstanding: everything this device was shown of this kind has been dealt with
        // somewhere else — at the desk, in the TUI, on another phone. Taking it down is the whole
        // point of the payload, and it is the one case that shows nothing at all.
        //
        ...
        if (note?.text("count") == "0") {
            clearAgentNotification(applicationContext, kind)
            return
        }
        // Likewise `alert`: absent means v1, and v1 was always news.
        when (note?.optBoolean("alert", true) != false) {
            true -> postAgentNotification(applicationContext, kind, title, body, pane)
            false -> postAgentResync(applicationContext, kind, title, body, pane)
        }
    }
